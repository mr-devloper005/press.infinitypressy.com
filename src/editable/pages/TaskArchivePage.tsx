  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="press-grid border-b border-[#1a312c]/20 bg-[var(--slot4-surface-bg)]">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            ...
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm font-bold leading-7 text-[#1a312c]/65">
              Timely reporting, sharp perspectives, and media-ready stories organized for fast discovery.
            </p>
          </div>
        </section>

        <section className="border-b border-[#1a312c] bg-[var(--slot4-dark-bg)] text-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'text-[var(--slot4-accent-soft)]' : 'hover:text-[var(--slot4-accent-soft)]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'text-[var(--slot4-accent-soft)]' : 'whitespace-nowrap hover:text-[var(--slot4-accent-soft)]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[1180px] border-x border-[#1a312c]/20 bg-white lg:grid-cols-[1.75fr_0.75fr]">
            ...
            <div className="grid">
              <div className="border-b border-[#1a312c] bg-[var(--slot4-accent)] p-6 text-white">
        ...
        <section className="mx-auto max-w-[1180px] border-x border-[#1a312c]/20 bg-[var(--slot4-page-bg)] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          ...
          {remaining.length ? (
            ...
            <div className="grid self-start border-l border-t border-[#1a312c]/25 md:grid-cols-2">
            ...
            <aside className="self-start lg:sticky lg:top-5">
              <form action="/search" className="bg-[var(--slot4-panel-bg)] p-5">
                <label className="text-xs font-black uppercase tracking-[.16em]">Search</label>
                <div className="mt-3 flex border border-[#1a312c]/20 bg-white"><input name="q" className="min-w-0 flex-1 px-3 py-3 text-sm outline-none" /><button className="bg-[var(--slot4-accent)] px-4 text-xs font-black text-white">Search</button></div>
              </form>
              <div className="mt-5 bg-[var(--slot4-surface-bg)] p-5"><h2 className="text-2xl font-black tracking-[-.04em]">Recent posts</h2><div className="mt-4">{posts.slice(0, 7).map((post, index) => <Link key={`${post.id || post.slug}-recent`} href={`${basePath}/${post.slug}`} className="grid grid-cols-[24px_1fr] gap-3 border-t border-[#1a312c]/15 py-3 text-sm font-bold leading-5 first:border-t-0"><span className="text-[var(--slot4-accent)]">{String(index + 1).padStart(2, '0')}</span><span>{post.title}</span></Link>)}</div></div>
            </aside>
            </div>
          ) : !lead ? (
